...
class Transactions {
  constructor({ type, amount, title, message, status, currency }) {
    (this.type = type),
      (this.amount = amount),
      (this.title = title),
...
class TransactionTransfer extends Transactions {
  ...
  async createTransactionTransfer() {
    ...
    await walletServices.updateBalance(
      this.sender,
      this.currency,
      -this.amount,
      session
...
class TransactionRefund extends Transactions {
  constructor({ sender, partnerID, ...options }) {
    super(options);
    (this.sender = sender), (this.partnerID = partnerID);
  }
...
module.exports = {
  ...
  updateTransactionRefund: async (transactionData, session) => {
    const updateTransactionResult = await Transaction.updateOne(
      transactionData._id,
      { status: "refunded" },
      { session }
    );
  ...
  getTransactionDetails: async (transactionID, userID) => {
    const query = {
      $or: [
        { _id: transactionID, sender: userID },
        { _id: transactionID, receiver: userID },
      ],
    };
