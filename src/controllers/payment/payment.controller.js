...
module.exports = {
  ...
  confirmPayment: catchError(async (req, res) => {
    ...
    const transactionDataTemp =
      await transactionServices.getTransactionForPayment(transactionID);
    ...
    const resultApply = await voucherServices.applyVoucherPayment(
      transactionDataTemp,
      session,
      voucher_code,
      getCurrency._id
    ...
    await wallet.updateBalancePartner(
      transactionDataTemp.partnerID,
      getCurrency._id,
      amount,
      session
    );
    const transactionData = await transactionServices.updateCompletedPayment(
      amount,
      voucherID,
      req.user,
      transactionID,
  ...
  refundMoney: catchError(async (req, res) => {
    ...
    const transactionData = await transactionServices.getTransactionRefund(
      partner._id,
      orderID
    );
    ...
    await wallet.checkBalancePartner(
      partner._id,
  ...
  applyVoucher: catchError(async (req, res) => {
    ...
    await voucherServices.checkOwnVoucher(
