...
const cardServices = require("../../services/credit-card.services");
  ...
  getCardS: catchError(async (req, res, next) => {
    ...
    card.forEach((element) => {
      const decrypt = {
        _id: element._id,
        number:
          element.number.substring(0, 4) +
          "*".repeat(element.number.length - 8) +
          element.number.substring(
            element.number.length - 4,
