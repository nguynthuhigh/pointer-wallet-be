...
module.exports = {
  ...
  editProfile: catchError(async (req, res) => {
    ...
    const partner = await Partner.updateOne({ _id: req.partner }, data, {
      new: true,
    });
  ...
  getTransactions: catchError(async (req, res) => {
    ...
    const transactionData = await transactionServices.getTransactionsPartner(
      partnerID,
      page,
      pagesize
    );
    const page_count = Math.ceil(
      (await transactionServices.countTransactionsPartner(partnerID)) / pagesize
    );
    const data = {
