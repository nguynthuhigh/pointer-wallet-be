...
exports.voucherSchema = yup.object({
    ...
    exprirationDate: yup.date().required("Expiration date is required"),
    discountValue: yup.number().required("Discount value is required").min(1,"Discount value is more than 1").max(100,"Discount value is less than or equal to more than 100"),
    type: yup.string().oneOf(['discount_amount', 'discount_percent']).required("Type is required"),
